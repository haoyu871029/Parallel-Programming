...
/* mandelbrot set */
    ...
    int row_index = *id * rows_per_thread; //row_index 會代表某列資料寫入 image 的哪一列

    for (int j = height-1-*id; j>=0; j-=thread_num) {
    //ex. 10x10 image, 4 threads, thread 0: 9 5 1, thread 1: 8 4 0
    //一圈處理一列
...
//execute: time srun -n1 -c5 ./hw2a out_hw2a.png 10000 -2 2 -2 2 800 800
//judge: hw2a-judge (about 754s)