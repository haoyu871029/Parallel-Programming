...
__global__ void phase1(int *dd_matrix, int r, int matrix_s){
    __shared__ int sb_for_pro[bf][bf];
    ...
    sb_for_pro[   i][   j] = dd_matrix[i_in_dd * matrix_s + j_in_dd]; 
    ...
    dd_matrix[i_in_dd * matrix_s + j_in_dd] = sb_for_pro[   i][   j];  
}

__global__ void phase2(int *dd_matrix, int r, int matrix_s){
    __shared__ int sb_for_pvt[bf][bf];
    __shared__ int sb_for_pro[bf][bf];
    ...
    //pivot data
    sb_for_pvt[   i][   j] = dd_matrix[i_in_dd * matrix_s + j_in_dd];

    if (blockIdx.x == 0){ //同行那些
        i_in_dd = (blockIdx.y >= r) ? i + ((blockIdx.y+1)<<5) : i + (blockIdx.y<<5);
    }
    else { //blockIdx.x == 1
        j_in_dd = (blockIdx.y >= r) ? j + ((blockIdx.y+1)<<5) : j + (blockIdx.y<<5);
    }
...
__global__ void phase3(int *dd_matrix, int r, int matrix_s){
    __shared__ int sb_for_row[bf][bf];
    __shared__ int sb_for_col[bf][bf];
    __shared__ int sb_for_pro[bf][bf];

    int i = threadIdx.y;
    int j = threadIdx.x;

    int pvt_i_in_dd = i + (r<<5);
    int pvt_j_in_dd = j + (r<<5);

    int i_in_dd = (blockIdx.x >= r) ? i + ((blockIdx.x+1)<<5) : i + (blockIdx.x<<5);
    int j_in_dd = (blockIdx.y >= r) ? j + ((blockIdx.y+1)<<5) : j + (blockIdx.y<<5);

    //process data
    sb_for_pro[   i][   j] = dd_matrix[i_in_dd * matrix_s + j_in_dd];   
    
    //row data
    sb_for_row[   i][   j] = dd_matrix[i_in_dd * matrix_s + pvt_j_in_dd]; 
    
    //column data
    sb_for_col[   i][   j] = dd_matrix[pvt_i_in_dd * matrix_s + j_in_dd];
