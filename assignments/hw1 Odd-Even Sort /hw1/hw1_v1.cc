/* hw1 version 1: 一定會進 mergeAndSort 作處理 */
...
int main(int argc, char** argv) {
    ...
    while (global_swap!=0){
        ...
        if (!(rank&1) && rank!=last_process_rank){
            ...
            merged_start = merged.begin();
            merged_end = merged.begin() + actual_num;
        }
        else if (rank&1){
            MPI_Sendrecv(data, actual_num, MPI_FLOAT, rank-1, 0,
            ...
            merged_start = merged.begin() + recv_num_from_befo;
            merged_end = merged.end();
        }
        for (int i=0; merged_start != merged_end; i++){
            if (data[i] != *merged_start){
                data_changed = true;
                break;
            }
            merged_start++;
