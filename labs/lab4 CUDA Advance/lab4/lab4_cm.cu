/* lab4 coalesced memory version 1  */
...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    int x = blockIdx.x * blockDim.x + threadIdx.x; //(x,0) 代表的是 thread 在全部 threads 中的編號，也代表此條 thread 要處理第幾行的資料
    if (x >= width) 
        return;

    double val[Z][3];
    for (int y = 0; y < height; ++y) {//代表每條 thread 處理一行 pixels
...
int main(int argc, char **argv) {
    ...
    const int num_blocks = width / num_threads + 1; //dimGrid
