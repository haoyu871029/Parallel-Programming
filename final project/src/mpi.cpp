...
void Parser(string input_file, string output_file, int rank){
    ...
    cout << "rank " << rank << "'s cut_size: " << cut_size << endl;
    long long min_cut;
    MPI_Reduce(&cut_size, &min_cut, 1, MPI_LONG_LONG_INT, MPI_MIN, 0, MPI_COMM_WORLD);
    //MPI_Bcast(&min_cut, 1, MPI_LONG_LONG_INT, 0, MPI_COMM_WORLD); // broadcast min_cut to all processes

    if(rank == 0){
        cout << endl << "min_cut: " << min_cut << endl;
        ofstream MyWriteFile(output_file);

        MyWriteFile << "CutSize " << min_cut << endl;
...
int main(int argc, char** argv) {
    ...
    /* MPI declaration & initialization */
    int rank, size;
    MPI_Init(&argc, &argv);
    ...
    Parser(input_file, output_file, rank);
...
//compile in apollo: mpicxx -std=c++11 -O3 -lm -fopenmp -o ../bin/main main.cpp
//execute in apollo: srun -N2 -n4 -c4 ../bin/main ../testcase/public5.txt ../output/public5.out
